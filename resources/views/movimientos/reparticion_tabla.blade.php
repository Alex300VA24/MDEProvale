...
<div class="bg-white rounded-2xl border-2 border-wheat shadow-sm overflow-hidden">
    <div class="flex items-center justify-between px-6 py-5 border-b-2 border-wheat flex-wrap gap-4">
        ...
        <div class="flex gap-3">
            <form method="GET" action="{{ route('movimientos.reparticion-tabla') }}" class="flex gap-2 items-center">
        ...
        <table class="w-full text-sm">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-4 py-3 text-center font-bold text-earth bg-leaf-light text-leaf">Leche</th>
                    <th class="px-4 py-3 text-center font-bold text-earth">Cajas</th>
                    <th class="px-4 py-3 text-center font-bold text-earth">Tarros</th>
                    <th class="px-4 py-3 text-center font-bold text-earth bg-sun-light text-[#D97706]">Hojuelas</th>
                    <th class="px-4 py-3 text-center font-bold text-earth">Sacos</th>
                    <th class="px-4 py-3 text-center font-bold text-earth">Kilos</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3 text-center font-bold text-leaf">{{ round($club['leche_litros']) }}</td>
                    <td class="px-4 py-3 text-center">{{ $club['leche_cajas'] ?? 0 }}</td>
                    <td class="px-4 py-3 text-center">{{ $club['leche_tarros'] ?? 0 }}</td>
                    <td class="px-4 py-3 text-center font-bold text-[#D97706]">{{ round($club['hojuelas_kg']) }}</td>
                    <td class="px-4 py-3 text-center">{{ $club['hojuelas_sacos'] ?? 0 }}</td>
                    <td class="px-4 py-3 text-center">{{ $club['hojuelas_kilos'] ?? 0 }}</td>
                </tr>
                @empty
                <tr>
            ...
            <tfoot class="bg-gray-100 font-bold">
                <tr>
                    ...
                    <td class="px-4 py-3 text-center text-leaf">{{ round($totalLecheLitros) }}</td>
                    <td class="px-4 py-3 text-center">{{ intdiv((int) round($totalLecheLitros), 48) }}</td>
                    <td class="px-4 py-3 text-center">{{ (int) round($totalLecheLitros) % 48 }}</td>
                    <td class="px-4 py-3 text-center text-[#D97706]">{{ round($totalHojuelasKg) }}</td>
                    <td class="px-4 py-3 text-center">{{ intdiv((int) round($totalHojuelasKg), 30) }}</td>
                    <td class="px-4 py-3 text-center">{{ (int) round($totalHojuelasKg) % 30 }}</td>
                </tr>
            </tfoot>
        </table>
    </div>

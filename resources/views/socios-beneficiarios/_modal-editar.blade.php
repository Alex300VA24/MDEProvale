{{-- Modal Editar Socio --}}
<div id="modal-editar-socio-{{ $partner->id }}" class="modal-overlay">
    <div class="modal-panel sm:max-w-4xl">
        <div class="modal-card">
            <div class="modal-header">
                ...
                <button onclick="closeModal('modal-editar-socio-{{ $partner->id }}')" class="modal-close-btn">
                    <i class="fas fa-times"></i>
                </button>
            </div>
            <form action="{{ route('socios-beneficiarios.socios.update', $partner) }}" method="POST" class="p-4 sm:p-6 space-y-4">
                @csrf
                @method('PUT')
                <div>
                    <label class="block text-[11px] font-bold text-earth uppercase tracking-wider mb-1">Persona *</label>
                    <select name="person_id" class="w-full px-4 py-2.5 border-2 border-wheat rounded-xl text-xs sm:text-sm font-semibold text-charcoal bg-white focus:outline-none focus:border-leaf transition-all" required>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" name="date_begin" value="{{ $partner->date_begin }}" class="w-full px-4 py-2.5 border-2 border-wheat rounded-xl text-xs sm:text-sm font-semibold text-charcoal bg-white focus:outline-none focus:border-leaf transition-all" required>
                    </div>
                    <div>
                        <label class="block text-[11px] font-bold text-earth uppercase tracking-wider mb-1">Fecha Fin *</label>
                        <input type="date" name="date_end" value="{{ $partner->date_end }}" class="w-full px-4 py-2.5 border-2 border-wheat rounded-xl text-xs sm:text-sm font-semibold text-charcoal bg-white focus:outline-none focus:border-leaf transition-all" required>
                    </div>
                </div>
                <div>
                ...
                <div>
                    ...
                    <textarea name="observations" rows="2" class="w-full px-4 py-2.5 border-2 border-wheat rounded-xl text-xs sm:text-sm font-semibold text-charcoal bg-white focus:outline-none focus:border-leaf transition-all">{{ $partner->observations }}</textarea>
                </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
                                <div>
                                    <label class="block text-[10px] font-bold text-earth uppercase mb-1">Persona *</label>
                                     <select name="beneficiaries[{{ $index }}][person_id]" class="w-full px-3 py-2 border-2 border-wheat rounded-lg text-xs sm:text-sm font-semibold bg-white" required>
                                        @foreach($allPeople as $person)
                                ...
                                <div>
                                    <label class="block text-[10px] font-bold text-earth uppercase mb-1">Parentesco *</label>
                                     <select name="beneficiaries[{{ $index }}][relationship_id]" class="w-full px-3 py-2 border-2 border-wheat rounded-lg text-xs sm:text-sm font-semibold bg-white" required>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-3">
                                <div>
                                    <label class="block text-[10px] font-bold text-earth uppercase mb-1">Peso (kg)</label>
                                     <input type="number" step="0.01" min="0" name="beneficiaries[{{ $index }}][weight]" value="{{ $latestHistory->weight ?? '' }}" class="w-full px-3 py-2 border-2 border-wheat rounded-lg text-xs sm:text-sm font-semibold bg-white" placeholder="65.50">
                                </div>
                                <div>
                                    <label class="block text-[10px] font-bold text-earth uppercase mb-1">Talla (cm)</label>
                                     <input type="number" step="0.01" min="0" name="beneficiaries[{{ $index }}][height]" value="{{ $latestHistory->height ?? '' }}" class="w-full px-3 py-2 border-2 border-wheat rounded-lg text-xs sm:text-sm font-semibold bg-white" placeholder="160.00">
                                </div>
                                <div>
                                    <label class="block text-[10px] font-bold text-earth uppercase mb-1">HMG (g/dL)</label>
                                     <input type="number" step="0.01" min="0" name="beneficiaries[{{ $index }}][hmg]" value="{{ $latestHistory->hmg ?? '' }}" class="w-full px-3 py-2 border-2 border-wheat rounded-lg text-xs sm:text-sm font-semibold bg-white" placeholder="12.50">
                                </div>
                            </div>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
                                <div>
                                    <label class="block text-[10px] font-bold text-earth uppercase mb-1">F. Inicio Beneficio</label>
                                     <input type="date" name="beneficiaries[{{ $index }}][date_begin]" value="{{ $latestHistory->date_begin ?? '' }}" class="w-full px-3 py-2 border-2 border-wheat rounded-lg text-xs sm:text-sm font-semibold bg-white">
                                </div>
                                <div>
                                    <label class="block text-[10px] font-bold text-earth uppercase mb-1">F. Fin Beneficio</label>
                                     <input type="date" name="beneficiaries[{{ $index }}][date_end]" value="{{ $latestHistory->date_end ?? '' }}" class="w-full px-3 py-2 border-2 border-wheat rounded-lg text-xs sm:text-sm font-semibold bg-white">
                                </div>
                            </div>
                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                <div>
                                    <label class="block text-[10px] font-bold text-earth uppercase mb-1">Tipo de Beneficio</label>
                                     <select name="beneficiaries[{{ $index }}][type_benefit_id]" class="w-full px-3 py-2 border-2 border-wheat rounded-lg text-xs sm:text-sm font-semibold bg-white">
                                ...
                                <div>
                                    <label class="block text-[10px] font-bold text-earth uppercase mb-1">Estado</label>
                                     <select name="beneficiaries[{{ $index }}][history_state_id]" class="w-full px-3 py-2 border-2 border-wheat rounded-lg text-xs sm:text-sm font-semibold bg-white">
                                        <option value="">Seleccionar...</option>
                                ...
                                <div>
                                    <label class="block text-[10px] font-bold text-earth uppercase mb-1">Motivo Descalificación</label>
                                     <select name="beneficiaries[{{ $index }}][reason_disqualification_id]" class="w-full px-3 py-2 border-2 border-wheat rounded-lg text-xs sm:text-sm font-semibold bg-white">

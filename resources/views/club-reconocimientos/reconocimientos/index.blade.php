...
<div class="bg-white rounded-2xl border-2 border-wheat shadow-sm overflow-hidden">
    <div class="flex items-center justify-between px-6 py-5 border-b-2 border-wheat flex-wrap gap-4">
        <h3 class="font-extrabold text-charcoal text-xl flex items-center gap-3">
            <i class="fas fa-file-contract text-leaf"></i> Gestión de Resoluciones
        </h3>
    ...
    <form method="GET" action="{{ route('club-reconocimientos.reconocimientos.index') }}" class="flex gap-4 px-6 py-4 flex-wrap border-b border-wheat bg-cream">
        ...
        <table class="data-table w-full">
            <thead>
                <tr>
                    ...
                    <th class="px-6 py-4 text-left">Comité</th>
            ...
            <tbody>
                ...
                <tr>
                    <td class="px-6 text-earth font-mono text-sm">#{{ $resolution->id }}</td>
                    <td class="px-6 font-semibold">{{ $resolution->document ?? 'Sin documento' }}</td>
                    <td class="px-6">
                        @if($resolution->associations->first())
                            <span class="px-2 py-1 rounded-lg bg-leaf-light text-leaf text-xs font-bold">{{ $resolution->associations->first()->name }}</span>
                        @else
                            -
                        @endif
                    </td>
                    <td class="px-6 text-earth text-sm">
                        {{ $resolution->date_document ? \Carbon\Carbon::parse($resolution->date_document)->format('d/m/Y') : '-' }}
                    </td>
                    <td class="px-6 text-sm text-earth">
                        {{ $resolution->date_start ? \Carbon\Carbon::parse($resolution->date_start)->format('d/m/Y') : '-' }} - 
                        {{ $resolution->date_end ? \Carbon\Carbon::parse($resolution->date_end)->format('d/m/Y') : '-' }}
                    </td>
                    <td class="px-6">
                        @if($resolution->state)
                            @if($resolution->state->title == 'Activo')
                                <span class="badge-active px-3 py-1 rounded-full text-xs font-bold">Activo</span>
                            @else
                                <span class="badge-inactive px-3 py-1 rounded-full text-xs font-bold">{{ $resolution->state->title }}</span>
                            @endif
                        @else
                    ...
                    <td class="px-6">
                        <div class="flex gap-2">
                            <a href="{{ route('club-reconocimientos.reconocimientos.show', $resolution) }}" class="btn-action bg-sky-light text-[#0284C7] hover:bg-sky hover:text-white">
                                <i class="fas fa-eye"></i>
                            </a>
                            <a href="{{ route('club-reconocimientos.reconocimientos.edit', $resolution) }}" class="btn-action bg-sun-light text-[#D97706] hover:bg-sun hover:text-white">
                                <i class="fas fa-edit"></i>
                            </a>
                            <form action="{{ route('club-reconocimientos.reconocimientos.destroy', $resolution) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn-action bg-clay-light text-clay hover:bg-clay hover:text-white" onclick="return confirm('¿Estás seguro de eliminar esta resolución?')">
    ...
    <div class="flex items-center justify-between px-6 py-4 border-t-2 border-wheat">
        <span class="text-sm text-earth font-medium">Mostrando {{ $resolutions->firstItem() ?? 0 }} - {{ $resolutions->lastItem() ?? 0 }} de {{ $resolutions->total() }} registros</span>
        {{ $resolutions->appends(request()->query())->links() }}
    </div>
</div>
@endsection

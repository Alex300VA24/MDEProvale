...
<div class="bg-white rounded-2xl border-2 border-wheat shadow-sm overflow-hidden">
    <div class="flex items-center justify-between px-6 py-5 border-b-2 border-wheat flex-wrap gap-4">
        ...
        <div class="flex gap-3">
            <a href="{{ route('beneficiarios.reportes') }}" class="btn-secondary flex items-center gap-2">
                <i class="fas fa-file-pdf"></i> Reportes
            </a>
            <a href="{{ route('beneficiarios.create') }}" class="btn-primary flex items-center gap-2">
                <i class="fas fa-plus"></i> Nuevo Beneficiario
            </a>
        </div>
    </div>

    <form method="GET" action="{{ route('beneficiarios.index') }}" class="flex gap-4 px-6 py-4 flex-wrap border-b border-wheat bg-cream">
        ...
        <div class="flex items-end gap-2">
            ...
            <a href="{{ route('beneficiarios.index') }}" class="btn-secondary">
                ...
                <tr>
                    ...
                    <td class="px-6">
                        <div class="flex gap-2">
                            <a href="{{ route('beneficiarios.show', $beneficiarie) }}" class="btn-action bg-sky-light text-[#0284C7] hover:bg-sky hover:text-white">
                                <i class="fas fa-eye"></i>
                            </a>
                            <a href="{{ route('beneficiarios.edit', $beneficiarie) }}" class="btn-action bg-sun-light text-[#D97706] hover:bg-sun hover:text-white">
                                <i class="fas fa-edit"></i>
                            </a>
                            <form action="{{ route('beneficiarios.destroy', $beneficiarie) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
